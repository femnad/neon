...
RecvAppendResponses(Safekeeper *sk)
{
	...
	/* update commit_lsn */
	wp->commitLsn = GetAcknowledgedByQuorumWALPosition(wp);
	/*
	 * Send the new value to all safekeepers.
	 */
	if (wp->commitLsn > wp->lastSentCommitLsn)
	{
		BroadcastAppendRequest(wp);
		wp->lastSentCommitLsn = wp->commitLsn;
	}
...
HandleSafekeeperResponse(WalProposer *wp)
{
	...
	wp->api.process_safekeeper_feedback(wp, wp->commitLsn);

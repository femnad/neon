...
impl Service {
    ...
    pub async fn spawn(config: Config, persistence: Arc<Persistence>) -> anyhow::Result<Arc<Self>> {
        ...
        for tsp in tenant_shard_persistence {
            ...
            if let Some(generation_pageserver) = tsp.generation_pageserver {
                intent.attached = Some(NodeId(generation_pageserver as u64))
            }
    ...
    pub(crate) async fn re_attach(
        ...
        for (tenant_shard_id, new_gen) in incremented_generations {
            ...
            let Some(old_gen) = shard_state.generation else {
                ...
                anyhow::bail!("Generation must be set while re-attaching");
            };
            ...
            for tenant_shard_id in create_ids {
                ...
                match locked.tenants.entry(tenant_shard_id) {
                    Entry::Occupied(mut entry) => {
                        ...
                        if let Some(node_id) = entry.get().intent.attached {
                    ...
                    Entry::Vacant(entry) => {
                        ...
                        if let Some(node_id) = state.intent.attached {
    ...
    pub(crate) async fn tenant_location_config(
        ...
        match create_or_update {
            CreateOrUpdate::Create((create_req, placement_policy)) => {
                let create_resp = self.tenant_create(create_req, placement_policy).await?;
    ...
    pub(crate) async fn tenant_secondary_download(
        ...
        let targets = {
            ...
            {
                for node_id in &shard.intent.secondary {
                    let node = locked
                        .nodes
                        .get(node_id)
            ...
            if shard.intent.attached == Some(migrate_req.node_id) {
                ...
                match shard.policy {
                    ...
                    PlacementPolicy::Secondary => {
                        shard.intent.secondary = vec![migrate_req.node_id];
                    }
